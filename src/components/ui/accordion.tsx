...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between gap-4 rounded-xl px-6 py-4 text-left text-base font-medium transition-all hover:text-primary [&[data-state=open]_.icon]:rotate-180",
        className
      )}
      {...props}
    >
      {children}
      <ChevronDown className="icon h-4 w-4 shrink-0 transition-transform duration-200" />
    </AccordionPrimitive.Trigger>
